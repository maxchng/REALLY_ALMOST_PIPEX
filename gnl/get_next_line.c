...
/*   Updated: 2023/11/08 03:05:06 by ychng            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*partial_buffer;
	char		*result_buffer;

	if (BUFFER_SIZE <= 0 || fd < 0)
		return (NULL);
	partial_buffer = read_file_into_buffer(fd, partial_buffer);
	result_buffer = extract_before_newline(partial_buffer);
	partial_buffer = extract_after_newline(partial_buffer);
	return (result_buffer);
}
